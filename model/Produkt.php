...
class Produkt implements DatabaseObject
{
    ...
    public static function getLetzterEintrag(){
        $db = Database::connect();
        $sql = 'SELECT produkt_id FROM produkt ORDER BY produkt_id DESC LIMIT 1';
        $stmt = $db->prepare($sql);
        $stmt->execute();
        $item = $stmt->fetchObject('Produkt');
        Database::disconnect();
    ...
    public static function getTeuerstesProdukt(){
        $db = Database::connect();
        $sql = 'SELECT * FROM produkt ORDER BY produkt_preis DESC LIMIT 1';
        $stmt = $db->prepare($sql);
        $stmt->execute();
        $item = $stmt->fetchObject('Produkt');
        Database::disconnect();
    ...
    public static function getBilligstesProdukt(){
        $db = Database::connect();
        $sql = 'SELECT * FROM produkt ORDER BY produkt_preis ASC LIMIT 1';
        $stmt = $db->prepare($sql);
        $stmt->execute();
        $item = $stmt->fetchObject('Produkt');
        Database::disconnect();
    ...
    public static function getMeistGelagertesProdukt(){
        $db = Database::connect();
        $sql = 'SELECT * FROM produkt ORDER BY produkt_menge DESC LIMIT 1';
        $stmt = $db->prepare($sql);
        $stmt->execute();
        $item = $stmt->fetchObject('Produkt');
        Database::disconnect();
    ...
    public static function getWenigstesGelagertesProdukt(){
        $db = Database::connect();
        $sql = 'SELECT * FROM produkt ORDER BY produkt_menge ASC LIMIT 1';
        $stmt = $db->prepare($sql);
        $stmt->execute();
        $item = $stmt->fetchObject('Produkt');
        Database::disconnect();
